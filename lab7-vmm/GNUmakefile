...
GULDFLAGS := -T user/guestuser.ld

UGCC_LIB := $(shell $(CC) $(CFLAGS) -print-libgcc-file-name)
...
# make it so that no intermediate .o files are ever deleted
.PRECIOUS: %.o $(GUESTDIR)/boot/%.o $(GUESTDIR)/kern/%.o \
	   $(GUESTDIR)/lib/%.o $(GUESTDIR)/fs/%.o $(GUESTDIR)/net/%.o \
	   $(GUESTDIR)/user/%.o
...
GUEST_KERN_CFLAGS := $(CFLAGS) -DJOS_KERNEL -DDWARF_SUPPORT -gdwarf-2 -mcmodel=large -m64
GUEST_USER_CFLAGS := $(CFLAGS) -DJOS_USER -gdwarf-2 -mcmodel=large -m64

GUEST_KERN_CFLAGS += -DVMM_GUEST
GUEST_USER_CFLAGS += -DVMM_GUEST
...
# Update .vars.X if variable X has changed since the last make run.
#
# Rules that use variable X should depend on $(GUESTDIR)/.vars.X.  If
# the variable's value has changed, this will update the vars file and
# force a rebuild of the rule that depends on it.
$(GUESTDIR)/.vars.%: FORCE
	$(V)echo "$($*)" | cmp -s $@ || echo "$($*)" > $@
.PRECIOUS: $(GUESTDIR)/.vars.%
.PHONY: FORCE
...
guestvm: $(GUESTDIR)/kern/kernel $(GUESTDIR)/boot/boot
...
# and keeps those dependencies up-to-date every time we recompile.
# See 'mergedep.pl' for more information.
$(GUESTDIR)/.deps: $(foreach dir, $(GUESTDIRS), $(wildcard $(GUESTDIR)/$(dir)/*.d))
	@mkdir -p $(@D)
	@$(PERL) mergedep.pl $@ $^

-include $(GUESTDIR)/.deps
