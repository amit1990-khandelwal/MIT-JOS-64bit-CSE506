...
// Sys_Call to transmit data
static int
sys_net_transmit(char* data, int len)
{
	/* Pointer check from the User space. */
	if ((uintptr_t) data >= UTOP)
		return -E_INVAL;

	return net_pci_transmit(data, len);
}

//Sys_Call to receive data
static int
...
syscall(uint64_t syscallno, uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5)
{
	...
	case SYS_net_transmit:
		cprintf("\n syscall len %d, address %x \n", (int)a2, a1);
		return sys_net_transmit((char*)a1, (int)a2);
	case SYS_net_receive:
		return sys_net_receive((char*)a1, (int*) a2);
	default:
		return -E_INVAL;
	}
}
